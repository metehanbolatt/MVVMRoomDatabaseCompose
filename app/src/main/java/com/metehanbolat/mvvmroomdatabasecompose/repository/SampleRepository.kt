...
class SampleRepository(private val sampleDao: SampleDao) {
    ...
    suspend fun addSample(item: List<SampleEntity>) = sampleDao.insert(item)

    suspend fun updateSample(item: SampleEntity) = sampleDao.update(item)

    suspend fun deleteSample(item: SampleEntity) = sampleDao.delete(item)
